...
function requestLogger(req, res, next) {
  ...
  res.on("finish", () => {
    ...
    console.log(
      `[API_LOG] ${req.method} ${req.path} - ${res.statusCode} (${duration}ms)`
    );

    // Log errors for specific endpoints that were failing
    if (
      res.statusCode >= 400 &&
      (req.path.includes("/request-token") || req.path.includes("/logo"))
    ) {
      console.warn(
        `[API_WARNING] Failed request to ${req.path} - Status: ${res.statusCode}`
      );
    }
  });
